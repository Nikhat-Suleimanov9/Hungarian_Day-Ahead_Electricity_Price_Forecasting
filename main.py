...
from src.feature import demand_minus_solar,get_calendar_features,add_rolling_stats,add_lags,add_fourier_features,drop_unused_features
...
def main():
    ...
    df = drop_unused_features(df,['day','month','year'])

    # Tuning, if need to find good hyperparameters
    #models, best_model_params = tuning(df,alphas = [0.0001, 0.01,0.05, 0.1, 1, 10], l1_ratios = [0.3, 0.4, 0.5, 0.7, 0.8, 0.9, 0.95],days_for_test_left = 45) # days_for_test_left needed to separate train-val from test, which gives here test set of size of 45 days 
