...
from src.data import convert_to_cet, dst_handling 
from src.feature import demand_minus_solar,get_calendar_features,add_rolling_stats,add_lags,add_fourier_features,drop_unused_features,corr_filtering,cross_intreactions,calc_ramps
...
def main():
    ...
    #models, best_model_params = tuning(df,alphas = [0.0001, 0.01,0.05, 0.1, 1], l1_ratios = [0.3, 0.4, 0.5, 0.7, 0.8, 0.9],days_for_test_left = days_for_test_left,n_days_valid=n_days_valid, test_size=1*24) # days_for_test_left needed to separate train-val from test, which gives here test set of size of 45 days 
